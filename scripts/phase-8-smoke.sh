...
check "SECONDARY order refused for invoicing" 409 \
  "$(code -X POST "$API/invoices/from-order/$SEC" -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d '{}')"
...
check "re-issuing an ISSUED invoice refused" 409 \
  "$(code -X POST "$API/invoices/$PAID/issue" -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d '{}')"
...
P=$(curl -s -X POST "$API/payments" -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
...
check "allocating a RECORDED receipt refused" 409 \
  "$(code -X POST "$API/payments/$P/allocate" -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "$ALLOC_BODY")"
check "self-verification refused" 403 \
  "$(code -X POST "$API/payments/$P/verify" -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d '{}')"
check "verification by another user permitted" 201 \
  "$(code -X POST "$API/payments/$P/verify" -H "Authorization: Bearer $FM" -H 'Content-Type: application/json' -d '{}')"
